    ...
    if (type == BOOK) {
      ...
      void Platform::write_back_file() {
        ...
        if (!fout.is_open()) {
          ...
          for (int i = 0; i < all_seller.size(); i++)
            fout << all_seller[i].GetUserType() << " "
                 << all_seller[i].GetName() << " " << all_seller[i].GetPswd()
                 << " " << all_seller[i].GetBalence() << " "
                 << all_seller[i].Getid() << endl;

          for (int i = 0; i < all_consumer.size(); i++)
            fout << all_consumer[i].GetUserType() << " "
                 << all_consumer[i].GetName() << " "
                 << all_consumer[i].GetPswd() << " "
                 << all_consumer[i].GetBalence() << " "
                 << all_consumer[i].Getid() << endl;
        }
        fout.close();
        ...
        if (!fout.is_open()) {
          ...
          for (int i = 0; i < all_food.size(); i++) {
            fout << all_food[i].GetProductType() << " "
                 << all_food[i].GetDscrip() << " " << all_food[i].GetPrice()
                 << " " << all_food[i].GetStock() << " " << all_food[i].GetId()
                 << " " << all_food[i].GetSellerId() << endl;
          }
          for (int i = 0; i < all_cloth.size(); i++) {
            fout << all_cloth[i].GetProductType() << " "
                 << all_cloth[i].GetDscrip() << " " << all_cloth[i].GetPrice()
                 << " " << all_cloth[i].GetStock() << " "
                 << all_cloth[i].GetId() << " " << all_cloth[i].GetSellerId()
                 << endl;
            for (int i = 0; i < all_book.size(); i++) {
              fout << all_book[i].GetProductType() << " "
                   << all_book[i].GetDscrip() << " " << all_book[i].GetPrice()
                   << " " << all_book[i].GetStock() << " "
      ...
      */
          ...
          for (int i = 0; i < all_book.size(); i++) {
            if (all_book[i].GetDscrip().find(input) != string::npos ||
              ...
              cout << setw(FORMAT_NAME_WID) << all_book[i].GetDscrip()
                   << setw(FORMAT_PRICE_WID) << all_book[i].GetPrice()
                   << setw(FORMAT_PRICE_WID) << all_book[i].GetId()
                   << setw(FORMAT_PRICE_WID) << all_book[i].GetSellerId()
                   << endl;
            }
          }
          for (int i = 0; i < all_cloth.size(); i++) {
            if (all_cloth[i].GetDscrip().find(input) != string::npos ||
              ...
              cout << setw(FORMAT_NAME_WID) << all_cloth[i].GetDscrip()
                   << setw(FORMAT_PRICE_WID) << all_cloth[i].GetPrice()
                   << setw(FORMAT_PRICE_WID) << all_cloth[i].GetId()
                   << setw(FORMAT_PRICE_WID) << all_cloth[i].GetSellerId()
                   << endl;
            }
          }
          for (int i = 0; i < all_food.size(); i++) {
            ...
            if (all_food[i].GetDscrip().find(input) != string::npos ||
              ...
              cout << setw(FORMAT_NAME_WID) << all_food[i].GetDscrip()
                   << setw(FORMAT_PRICE_WID) << all_food[i].GetPrice()
                   << setw(FORMAT_PRICE_WID) << all_food[i].GetId()
                   << setw(FORMAT_PRICE_WID) << all_food[i].GetSellerId()
                   << endl;
            }
          }
          if (!flag)
      ...
      void Platform::change_password() {
        ...
        cin >> new_pswd;
        if (cur_account->changePassword(old_pswd,
                                        new_pswd)) // oldpassowrd is corrct
        {
                    ...
                    switch (type) {
                      if (type == FOOD) {
                        ...
                        void Platform::edt_pdt() {
                          ...
                          print_my_product(BOOK);
                          print_my_product(CLOTH);
                          cout << "enter type.1 descrip,2 price,3 stock"
                               << endl;
                          cin >> type;
                          ...
                          search_my_pdt(pdt_type, id, cur_pdt,
                                        (seller *)cur_account);
                          if (price > 0) {
                        ...
                        void Platform::search_my_pdt(
                          ...
                          switch (type) {
                            ...
                            if (type == FOOD) {
                              it_f = all_food.begin();
                              while (it_f->GetSellerId() !=
                                     cur_account->Getid()) {
                                it_f++;
                              }

                              for (;
                                   it_f->GetSellerId() == cur_account->Getid();
                                   it_f++) {
                                if (it_f->GetId() == id) {
                                  cur_pdt = it_f;
                                  break;
                              ...
                              if (type == BOOK) {
                                it_b = all_book.begin();
                                while (it_b->GetSellerId() !=
                                       cur_account->Getid()) {
                                  it_b++;
                                }

                                for (; it_b->GetSellerId() ==
                                       cur_account->Getid();
                                     it_b++) {
                                  if (it_b->GetId() == id) {
                                    cur_pdt = it_b;
                                ...
                                if (type == CLOTH) {
                                  ...
                                  while (it_c->GetSellerId() !=
                                         cur_account->Getid()) {
                                    it_c++;
                                  }

                                  for (; it_c->GetSellerId() ==
                                         cur_account->Getid();
                                       it_c++) {
                                    if (it_c->GetId() == id) {
                                      cur_pdt = it_c;
                              ...
                              void Platform ::print_my_product(int type) {
                                ...
                                switch (type) {
                                  if (type == FOOD) {
                                    ...
                                    if (type > 0) {
                                      cout << setw(FORMAT_BAR_FRONT)
                                           << setfill('-') << "PRINTING MY FOOD"
                                           << setw(FORMAT_BAR_RARE) << ""
                                           << endl
                                           << setfill(' ');

                                      cout << setw(FORMAT_NAME_WID) << "Name"
                                           << setw(FORMAT_PRICE_WID) << "Price"
                                           << setw(FORMAT_PRICE_WID) << "STOCK"
                                           << setw(FORMAT_PRICE_WID) << "ID"
                                           << setw(FORMAT_PRICE_WID)
                                           << "SELLER_ID" << endl;
                                      for (; it_f->GetSellerId() ==
                                             cur_account->Getid();) {
                                        cout << setw(FORMAT_NAME_WID)
                                             << it_f->GetDscrip()
                                             << setw(FORMAT_PRICE_WID)
                                             << it_f->GetPrice()
                                             << setw(FORMAT_PRICE_WID)
                                             ...
                                             << it_f->GetId()
                                             << setw(FORMAT_PRICE_WID)
                                             << it_f->GetSellerId() << endl;
                                        cout << setw(FORMAT_BAR_FRONT)
                                             << setfill('-') << "END OF PRINT"
                                             << setw(FORMAT_BAR_RARE) << ""
                                             << endl
                                             << setfill(' ');
                                        it_f++;
